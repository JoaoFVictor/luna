...
export function buildNativeWorkflowExecutors({
  ...
  const providerBuiltIns = createNativeProviderBuiltIns({ workflowBuiltIns });
  ...
  const patternExecutors = qualityGatePatternExecutors;
...
function executableCapabilityIds(
  ...
  return capabilityRegistry
    .orderedManifests()
    .flatMap((manifest) =>
      Object.values(manifest[kind] ?? {}).map((registration) => registration.id)
    );
}
