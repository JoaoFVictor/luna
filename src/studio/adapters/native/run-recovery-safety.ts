...
function nativeStudioEffectsAreSafe(
  ...
  return job.preallocation.side_effects.every((raw) => {
    ...
    if (effect.stage === "potential") {
      ...
      return parsed.success && allowsEffect(parsed.data);
    }
    ...
    if (effect.stage === "uncertainty") {
      ...
      return parsed.success && !parsed.data.may_include_unlisted_write;
    }
    return false;
  });
}
